...
- `docs/handbook/` for process/tooling
...
2. **Open questions live in docs.** Use `🙋‍♂️/🤖/✅` comments with block IDs. See [[handbook/10-docs/10.01-open-questions-system]].
...
4. **Johnny Decimal (two-digit).** Use `NN.NN` for handbook/reference IDs.

## One-Shot Usage (LLM)

1. Read `docs/AGENTS.md` and the closest folder `AGENTS.md`.
2. Update handbook/reference docs, not sidecars.
