...
const TkToolParams = Type.Object({
  args: Type.Optional(
    Type.String({
      description:
        "Arguments to pass to the tk CLI. Use 'ls' (not 'list'), flags with = (e.g. --status=open). Examples: 'ls --status=open', 'show abc', 'start abc'.",
    }),
  ),
...
const TodosOneshotParams = Type.Object({
  ...
  type: Type.Optional(
    Type.String({ description: "Ticket type (bug|feature|task|epic|chore). Default: task." }),
  ),
  ...
  acceptanceCriteria: Type.Optional(
    Type.Array(Type.String({ description: "Acceptance criteria item." })),
  ),
...
const createTicketOneshot = async (
  ...
  if (createRes.code !== 0) {
    const out = [createRes.stdout, createRes.stderr]
      .filter(Boolean)
      .join(createRes.stderr ? "\n" : "");
...
const renderTkMessage = (
  message: { content: string; details?: TkMessageDetails },
  theme: Theme,
): Text => {
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    name: "todos",
    label: "Todos (tk)",
    description: "Run tk commands - ls [--status=open] [-a X] [-T X], ready, blocked, closed, show/start/close/reopen <id>, add-note <id> [text], create [title] [-d desc] [-t type] [-p pri] [--tags a,b], dep/link <id> <id>",
